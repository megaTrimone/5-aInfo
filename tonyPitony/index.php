...
<div class="container">
    ...
    <table>
        <tr><th>Data</th><th>Cliente</th><th>Prodotto</th><th>Q.tà</th><th>Stato</th></tr>
        <?php
        ...
        if($res && $res->num_rows > 0) {
            while($row = $res->fetch_assoc()) {
                echo "<tr><td>{$row['data_ordine']}</td><td>{$row['nome']} {$row['cognome']}</td><td>{$row['nome_prodotto']}</td><td>{$row['quantita_ordinata']}</td><td>{$row['stato']}</td></tr>";
            }
